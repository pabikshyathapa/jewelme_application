...
class _LoginPageState extends State<LoginPage> {
  ...
  void _startShakeDetection() {
    ...
    final shakeCooldown = Duration(seconds: 2);
    DateTime lastShakeTime = DateTime.now().subtract(shakeCooldown);
    ...
    _accelerometerSubscription = accelerometerEvents.listen((event) {
      if (lastEvent != null) {
        ...
        double totalShake = sqrt(deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ);

        if (totalShake > shakeThreshold &&
            DateTime.now().difference(lastShakeTime) > shakeCooldown) {
          lastShakeTime = DateTime.now();
          _triggerLogin(); // Trigger login on shake
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Shake detected! Attempting login...")),
          );
        }
      }
      lastEvent = event;
